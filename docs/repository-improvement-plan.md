...
**Implemented on `codex/fix-level1-crawler-lighting`:** `Level1LightingBootstrap` raises the cool ambient floor and shadowless directional fill rather than making navigation depend on complete darkness. `CrawlerVisualController` now preserves Zombie Crawl's authored **world** scale when it is reparented under the already-scaled gameplay root, retunes the prototype to 12 m detection / 2.25 m/s patrol / 4.5 m/s chase / 0.1 s target checks, and drives Animator playback from measured root motion with root motion disabled. `TeleportGorillaPlayerPhotonVR` accepts contact from the local rig rather than only the camera collider while still requiring the Containment sector and `Level1_Vents`. `ZoneTrigger` also repairs the missing safe-room-to-vent transition on the second Level 1 route by using the tracked head's directional exit from the safe boundary. The local vent headlamp idea recorded in the design document remains **proposed only** and is not implemented by this branch.

**Source review after the correction:** the Crawler capture trigger is present and has a scaled capsule; the gameplay root still owns NavMesh movement, audio, capture and Photon state; Zombie Crawl's controller contains one crawl animation state; the scene has two `Level1_Antechamber` safe boundaries but only one separately-authored `Level1_Vents` trigger, which explains the stale-safe-zone failure on the second route. `SectorMonsterSync` continues to replicate root position/rotation/chase state and elect a controller from players currently in the level. The branch is cleanly ahead of `main`; there are currently no GitHub status checks or workflow runs proving Unity behavior.

**Additional source findings, not yet claimed fixed:** a reaching hand can still enter a Level 1 safe trigger before the tracked head/body and mark the player safe early; capture currently depends on an `OnTriggerEnter` event, so an overlap that begins while travel/zone state is ineligible can be missed if it never re-enters; the capture drop point is the collider that touched the Crawler rather than the Gorilla body position; Zombie Crawl lookup uses active/global `GameObject.Find`; and the imported crawl clip's loop flag could not be verified from source because the animation asset is large. These are hardening items for the next code pass. Do not mark them resolved until implemented and tested.

**Pending validation:** Unity 2022.3.55f1 compile/import; narrowest vent, 90-degree turn and junction clearance; both safe-room entrances in both directions; crawl-cycle appearance at patrol and chase speeds; actual world scale/head-hand-floor contact; local head/body/hand capture; held-card drop location; immediate pursuit stop on either safe room; return to patrol; two-client shared chase/controller handover; patrol/hunt audio; headset readability; and Quest performance. The current lighting values and 12 m / 2.25 / 4.5 tuning are prototype values, not final difficulty or art decisions.
...
These source changes are merged in PR #3, with runtime validation still pending. The September 10 audit adds startup retry and matchmaking coordination. Level 2 completion is now merged through PR #4. Zombie Crawl visual integration is now present in `main`; the September 11 runtime follow-up and remaining hardening are tracked at the top of this plan. Backend authentication security remains separate work.

The held-item implementation uses the XR Interaction Toolkit 2.6 [first-select and last-release events](https://docs.unity3d.com/Packages/com.unity.xr.interaction.toolkit@2.6/api/UnityEngine.XR.Interaction.Toolkit.XRBaseInteractable.html#UnityEngine_XR_Interaction_Toolkit.XRBaseInteractable_firstSelectEntered). Source checks and patch reconstruction do not establish runtime selection behavior. The hand-audio fixes apply to the existing two Bootstrap components; the held-item GUID is referenced once in Level1_Containment. No scene geometry or serialized gameplay settings were changed in this additional pass.
...
**Current status, 2026-09-11:** sector/vent eligibility and immediate patrol return are implemented through the merged travel work. `codex/fix-level1-crawler-lighting` adds prototype chase tuning (12 m range, 0.1 s checks, 2.25 m/s patrol, 4.5 m/s chase), fixes the second safe-room route's missing return-to-vent zone publication, and speed-links the Zombie Crawl Animator to actual root movement. Closest-player switching, the absent authored VentGraph, final NavMesh limits and headset difficulty remain open. The safe-boundary hand-overlap issue recorded in the Level 1 follow-up above is still pending hardening.
...
Current source now clears an ineligible target and returns to patrol rather than continuing toward an old safe-room destination. Keep this immediate recovery behavior. Decide separately whether to retain closest-player switching or lock an eligible target until escape/capture, and whether patrol should remain random or ordered.
...
P1 / confirmed code defects at the original baseline. The current `AudioScaler.StopIfPlaying` does call `source.Stop()`, and `PlayerVentState.LocalPlayerInVent` uses `ZoneStateService` while in a Photon room. `MonsterActivationGate` gates the local Animator/audio presentation by vent state while leaving shared `MonsterNavigation`/`SectorMonsterSync` simulation active. Runtime audio ownership and headset playback still need validation.

Fix/validation target: keep one reliable local vent/safe-room state and one audio owner. Explicitly stop or fade the previous loop on state change and sector exit. Drive patrol/hunt selection from the shared monster state. Keep simulation eligibility separate from local sound audibility.

Evidence: [Empty stop method at baseline](https://github.com/GregStephen/RunawayChimps/blob/4f6894141aa2b132d744cb2423c1ee227e0fc5be/Assets/Scripts/Audio/AudioScaler.cs#L125)  |  [Local binding baseline](https://github.com/GregStephen/RunawayChimps/blob/4f6894141aa2b132d744cb2423c1ee227e0fc5be/Assets/Scripts/PlayerScripts/PlayerVentState.cs#L10)  |  [Vent-state placement](https://github.com/GregStephen/RunawayChimps/blob/4f6894141aa2b132d744cb2423c1ee227e0fc5be/Assets/Scenes/Bootstrap.unity#L2068)  |  [Skipped reactor updates baseline](https://github.com/GregStephen/RunawayChimps/blob/4f6894141aa2b132d744cb2423c1ee227e0fc5be/Assets/Scripts/ProxManager/ProximityManager.cs#L111)
...
**Current status, 2026-09-11:** the guarded `SectorTravelService.RespawnAt` path is present, and `codex/fix-level1-crawler-lighting` broadens capture eligibility from only the `MainCamera` collider to colliders belonging to the authoritative local rig while retaining Containment/vent/busy guards. Source inspection confirms the scene capture trigger and scaled capsule are present. Two hardening items remain: a contact that first overlaps while the zone/travel guard is false can be missed because the script currently uses only `OnTriggerEnter`, and the held-card drop point currently comes from the contacting collider rather than the Gorilla body position. Both remain pending implementation/validation.
...
Current source uses the shared guarded movement foundation and PR #5 adds additional grounding/state cleanup. Acceptance still requires repeated/held trigger contact, card drop, fade recovery and headset locomotion testing.
...
**Current status, 2026-09-11:** Zombie Crawl is now the visible replacement integrated onto the existing Level 1 gameplay root, and `codex/fix-level1-crawler-lighting` contains the first runtime follow-up after Greg's play feedback. The gameplay root still owns navigation, capture, Photon sync, patrol/hunt audio and proximity behavior. The visual controller reparents Zombie Crawl, preserves its authored world scale under the root's non-unit scale, applies the mesh-facing correction to the visual, disables legacy renderers/animators, binds `MonsterActivationGate` to the Zombie Animator, disables root motion and derives crawl playback from actual navigation movement. Unity/headset validation and the additional hardening findings at the top of this plan remain pending.
...
- Keep navigation responsible for world movement and the Animator responsible only for crawl pose/cycle. Verify the imported crawl clip loops correctly or add a robust runtime restart if it does not.
- Replace global/active-only Zombie lookup with a same-scene search if further hardening is implemented, so future inactive authored visuals cannot silently fall back to the old rig.
...
| Crawler presentation | Zombie Crawl appears at the intended physical scale, stays aligned to the gameplay root/floor, visibly cycles its crawl while moving, does not flash/freeze into a sliding pose, and clears the narrowest straight, 90-degree turn and junction. |
...
| Safe-room escape | Enter either safe room with the tracked player, not merely a reaching hand: that player stops being a target. Leaving either safe room back into the vents restores `Level1_Vents`. With no other eligible target, the Crawler immediately resumes patrol. |
...
| Capture and floor recovery | Ordinary local head/body/hand contact while in the vents starts exactly one capture. Held overlap cannot become permanently ignored after a transient busy/zone state. A held card drops at the player's recorded capture position; out-of-bounds returns it to the fixed room spawn. The overlap retry/body-position hardening is still pending source work. |
