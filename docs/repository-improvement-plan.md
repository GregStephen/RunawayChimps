...
**Implemented on review branch `codex/level2-blockout`, rebased onto `main` at `c388d87`; not merged:** v0.3 contains a proposed 4 × 4 m room beneath the bypass, north-facing small gate, scanner and color/symbol sign, collectible plinth, optional currency-cache placeholder and closed barrier. It also completes the interrupted gate revision: three frame-only linked instances and one complete exit gate. Existing source prefabs, scenes, travel scripts, Build Settings and Photon configuration are unchanged.
