...
**Confirmed runtime feedback:** after testing the current Level 1 follow-up branch, Greg reported that the local player can still occasionally begin partially inside the Hub floor and that two Hub → Level 1 transitions produced severe arrival glitching. This is a spawn/travel reliability defect, not a change to the intended spawn locations or level flow.

**Source finding:** local startup placement and sector travel use two different rig-placement paths. `RigSpawnSnapper` previously waited only two frames plus one fixed tick, moved/grounded the rig once, then immediately restored Rigidbody physics without freezing the Gorilla locomotion behaviour or body/head colliders and without checking final body/head clearance. `SectorTravelService.PlaceRig` already froze locomotion/colliders and checked floor/body/head space, but performed only one placement pass before the transition eventually restored physics. XR tracking/body transforms can continue settling over subsequent fixed ticks, so a pose that was valid on the first pass can be slightly penetrated by the time collision/locomotion resume. `PlayerSpawner` only chooses the network-avatar pose; it is not responsible for the local Gorilla locomotion body's floor placement.

**Implemented on `codex/fix-level1-crawler-lighting`:** Hub startup now keeps the Gorilla locomotion behaviour, body/head colliders and Rigidbody collision response frozen while the XR rig is aligned to the scene-scoped `HubSpawn`; it repeatedly re-grounds over three fixed-physics settle steps, performs one final correction, resets locomotion caches, uses a 0.06 m floor skin, verifies both body and head clearance, and only then restores physics and marks startup ready. Sector travel/respawn now similarly repeats `PlaceRig` across three fixed ticks while the rig remains frozen, rechecking floor/body/head clearance each time and using a 0.06 m arrival ground skin before any colliders, interactors or locomotion behaviours are restored. These settle counts/skins are defensive prototype values and can be tuned only from measured runtime behavior.

**Pending validation:** Unity 2022.3.55f1 compile/import, then repeat cold Hub startup enough times to catch the former intermittent floor case (including headset standing/crouching and recentering), verify no startup obstruction error at the valid `HubSpawn`, and repeatedly travel Hub → Level 1, Level 1 → Hub, Level 1 completion → Level 2 and RETURN TO SECURITY while watching for floor penetration, launch/jitter, stuck hands, stale velocity or a delayed physics kick. Also repeat Crawler capture respawns because the same stabilized `RespawnAt` path is now used there. Two-client Photon and Quest headset validation remain required; source inspection alone does not prove the intermittent issue is resolved.
...
**Implemented on `codex/fix-level1-crawler-lighting`:** `Level1LightingBootstrap` raises the cool ambient floor and shadowless directional fill rather than making navigation depend on complete darkness. `CrawlerVisualController` preserves Zombie Crawl's authored **world** scale when it is reparented under the already-scaled gameplay root, retunes the prototype to 12 m detection / 2.25 m/s patrol / 4.5 m/s chase / 0.1 s target checks, and drives Animator playback from measured root motion with root motion disabled. `TeleportGorillaPlayerPhotonVR` accepts contact from the local rig rather than only the camera collider while still requiring the Containment sector and `Level1_Vents`. `ZoneTrigger` repairs the missing safe-room-to-vent transition on the second Level 1 route. The local vent headlamp idea recorded in the design document remains **proposed only** and is not implemented by this branch.
...
**Pending validation:** Unity 2022.3.55f1 compile/import; narrowest vent, 90-degree turn and junction clearance; both safe-room entrances in both directions; crawl-cycle appearance at patrol and chase speeds; actual world scale/head-hand-floor contact; local head/body/hand capture including a held-overlap case; held-card drop location; immediate pursuit stop on either safe room; return to patrol; two-client shared chase/controller handover; visibility of an animated Crawler from a safe-room doorway; patrol/hunt audio; headset readability; and Quest performance. The current lighting values and 12 m / 2.25 / 4.5 tuning are prototype values, not final difficulty or art decisions.
...
**Current status, 2026-09-11:** sector/vent eligibility and immediate patrol return are implemented through the merged travel work. `codex/fix-level1-crawler-lighting` adds prototype chase tuning (12 m range, 0.1 s checks, 2.25 m/s patrol, 4.5 m/s chase), repairs both safe-room directions using the tracked head as the Level 1 boundary authority, and speed-links the Zombie Crawl Animator to actual root movement. The shared Crawler's Animator now stays enabled even when the local viewer is safe, preventing a visible moving monster from becoming a frozen-pose slide. Closest-player switching, the absent authored VentGraph, final NavMesh limits and headset difficulty remain open.
...
**Current status, 2026-09-11:** Zombie Crawl is the visible replacement integrated onto the existing Level 1 gameplay root, and `codex/fix-level1-crawler-lighting` contains the first runtime follow-up after Greg's play feedback. The gameplay root still owns navigation, capture, Photon sync, patrol/hunt audio and proximity behavior. The visual controller reparents Zombie Crawl, preserves its authored world scale under the root's non-unit scale, applies the mesh-facing correction to the visual, disables legacy renderers/animators, binds `MonsterActivationGate` to the Zombie Animator, disables root motion and derives crawl playback from actual navigation movement. It now searches the loaded Level 1 scene including inactive objects, restarts a non-looping crawl state while moving, and stays animated independently of the local viewer's safe/vent audio gate. Unity/headset validation remains pending.

- Keep the gameplay root and its stable links; the visual replacement should remain independently adjustable for scale, orientation and floor contact.
...
- Keep navigation responsible for world movement and the Animator responsible only for crawl pose/cycle. Do not restore root motion.
- Tune body height and hand contact before further speed changes, then test the narrowest straight, a 90-degree turn, a junction and both safe-room entrances. Fit the long crawling body as well as the round NavMesh agent.
...
| 2 Crawler and chase Large | Finish/validate the September 11 Zombie Crawl runtime follow-up plus R02-R04: scale/crawl presentation, zone eligibility, navigation/audio, one network controller and state handover. | The Crawler visibly crawls at the intended scale, fits the vents on both clients, pursues eligible vent players, safe rooms end pursuit, patrol/sound recover, capture works, and controller departure is handled. |
...
Start from `codex/fix-level1-crawler-lighting` for this focused Level 1 pass. Run the offline source validator and both Runaway Chimps Editor validators in Unity 2022.3.55f1 before Play Mode. Resolve any compile/import/missing-script issue first. **First repeat cold Hub startup and Hub → Level 1 travel several times to validate the new multi-fixed-step rig settling before evaluating Crawler behavior.** Confirm no floor penetration, jitter/launch, stuck hands or post-fade physics kick, then continue with the brighter baseline, Zombie Crawl world scale/floor-hand contact, continuous crawl animation, patrol/chase speeds, both safe-room boundaries in both directions, capture from head/body/hand contact including held overlap, body-position card drops, immediate patrol return, safe-room doorway animation visibility, audio state and the narrowest vent turns. Repeat travel/Crawler checks with two Photon clients, including controller handover. The local vent headlamp remains optional/proposed and should not be used to hide a globally unreadable lighting baseline. Continue with Level 2, reconnect/pause and broader reliability checks after this Level 1 checkpoint is dependable.
...
| Spawn/arrival stability | Repeated cold Hub starts place the Gorilla capsule above the actual Hub floor without penetration or an obstruction error. Repeated sector travel and capture respawns keep the rig frozen while it settles, then restore body/head colliders, hands and locomotion without jitter, launch, stale velocity or a delayed physics kick. |
| Level 1 readability | In headset and non-headset Play Mode, walls, vent junctions, route edges and the Crawler are readable without turning the horror level into a brightly lit room. The proposed local headlamp is not required for basic navigation. |
| Crawler presentation | Zombie Crawl appears at the intended physical scale, stays aligned to the gameplay root/floor, visibly cycles its crawl while moving, does not flash/freeze into a sliding pose, and clears the narrowest straight, 90-degree turn and junction. A client in a safe room still sees an animated shared Crawler if it is visible through the vent opening. |
...
| Quest stability | No stale locomotion impulse, stuck fade, missing floor, duplicate rig, accumulated level objects, excessive lighting cost or new Crawler-frame spikes after repeated travel. Record frame-time and memory measurements. |
...
PR #5 adds Editor reliability regression checks and extends the travel validator to detect missing scripts. The `codex/fix-player-rig-tpose` branch now also repairs and asserts the Photon player visual-hand IK target/pole/view bindings and right-hand XR side before the reliability suite continues. Add focused Level 1 checks for both safe boundaries/vent restoration, the Zombie visual binding/world-scale compensation, capture trigger references and the absence of missing scripts. Add a startup/travel regression check that verifies `RigSpawnSnapper` and `SectorTravelService` retain nonzero settle steps/ground skin and that required spawn markers have usable floor/head/body clearance; source checks still cannot reproduce XR tracking timing. Continue adding small tests for the personal visit lifecycle and eligible-target transitions. When v0.4 geometry is merged, extend the Level 2 source/layout checks to enforce the new minimum footprint and required connectivity without confusing those source checks with Unity/NavMesh/headset validation. Use two-client and headset checks for network synchronization, fades, collision, voice, animation appearance and performance; source inspection cannot establish those outcomes.
...
- Level 1 presentation: the head-mounted vent light is proposed only. Decide after testing the corrected baseline whether it improves atmosphere/readability enough to become a confirmed feature.
