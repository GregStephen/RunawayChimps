...
**Implemented on PR #6 (`codex/hub-level1-physical-button`):** `SectorDoor` now suppresses direct `XRSimpleInteractable` selection and the generated select prompt specifically for Hub → Level 1 while preserving Level 1 → Hub XR selection. Source inspection found the existing `StartLevel1Button` root authored inactive in `Hub_Base`, which explains why it was not visible. Its `BigRedButton` children are positioned beside/before the gate, its visible mesh renderer and trigger collider are enabled, its `PhysicalButton` requires the local rig plus `HandTag`/hand layer, and its one persistent `OnPressed` call targets the Hub `SectorDoor.Travel` method. The Hub door now reactivates that button root at runtime before interaction.

**Validation added on PR #6:** `Validate Sector Travel` now checks that the button exists, has an enabled trigger and visible renderer, filters to the intended local hand, is wired exactly once to the exact Hub door, remains within 2.5 m of the gate, and stays on the Hub approach side. README route instructions were updated as well. Repository/source inspection is complete; Unity 2022.3.55f1 compilation/import, the Editor validator run, Play Mode hand-contact behavior, and headset validation remain pending. The scene file still carries the old inactive authoring flag, so outside Play Mode the button may remain hidden until that scene object is saved active in Unity; PR #6 guarantees runtime recovery and warns about the authored inactive state.
...
- **Implemented wiring in PR #3, corrected by PR #6:** the existing Hub button targets `SectorDoor.Travel` and requires a local hand; PR #3 also gave both scene entrance doors XR selection with solid barriers. PR #6 supersedes the Hub half of that dual-input design: `StartLevel1Button` is the only player-facing Hub hallway activation, direct Hub-gate XR selection is suppressed, and the existing authored-inactive button root is reactivated at runtime. The Level 1 return door remains unchanged unless separately redesigned. The Hub computer has a Level 1 menu option; keyboard buttons also require the local rig. The default Hub spawn is retained for startup; separate computer, hallway, and Level 1 entry markers are serialized. One persistent XR interaction manager replaces Level 1's scene-owned manager and rebinds arriving interactables.
...
- **Pending validation:** use `Tools > Runaway Chimps > Validate Sector Travel`, then execute the [route and multiplayer checks](../README.md#scene-travel-checks) in Unity 2022.3.55f1. Confirm PR #6's button-only Hub hallway interaction and runtime button recovery, scene extraction, navmesh activation, floor/head/body clearance, camera-space Loading in both eyes, voice, controller handover, capture, rollback, and repeated travel/memory. PR #5 adds reconnect/pause cases that must be included.
...
**Current status, 2026-09-11:** The missing gate target and scene split/travel wiring are implemented in merged PR #3. PR #6 implements the September 11 interaction correction: the physical `StartLevel1Button` is the only Hub hallway activation, the Hub gate rejects direct XR Select/Grip, and the authored-inactive button root is reactivated at runtime. Source inspection confirms the existing button's renderer, trigger, local-hand filter, placement beside/before the gate, and exact `SectorDoor.Travel` binding. Unity 2022.3.55f1 compilation, validator execution, Play Mode and headset hand-contact validation remain pending. The Level 1 → Hub return-door interaction is unchanged.
...
1. Verify the Hub-side `StartLevel1Button` remains clearly reachable and is the only Hub hallway activation after PR #6; the gate itself should not expose XR Select/Grip. The current scene file still has the button root authored inactive, so save it active when editing `Hub_Base` next; until then PR #6 reactivates it at runtime and the validator warns about the authored state.
...
Start from the current PR #6/PR #5 reconciled work rather than either pre-merge branch. Run the offline source validator. Open Unity 2022.3.55f1 and run both Runaway Chimps validators before Play Mode. Resolve any compile/import/missing-script issue first. Check PR #6's Hub entrance correction: confirm `StartLevel1Button` becomes visible before the gate, accepts the local hand/fingertip in headset play and when moving the monkey hand collider in non-headset editor testing, and leaves the Hub gate non-selectable. Continue with startup failure/retry, private/public room coordination, Hub → Level 1/Level 2, personal Level 1 completion → Level 2, RETURN TO SECURITY → Hub, failed-load rollback, reconnect/pause, and two-client sector/monster behavior. Keep Listener gameplay, reward-room persistence, shop expansion and camera-wall production work behind this dependable checkpoint.
...
PR #5 adds Editor reliability regression checks and extends the travel validator to detect missing scripts. PR #6 extends the travel validator for the button-only Hub entrance rule, including visibility, trigger/filter wiring, exact target, distance and approach-side placement. Continue adding small tests for the personal visit lifecycle and eligible-target transitions. Use two-client and headset checks for network synchronization, fades, collision, voice and performance; source inspection cannot establish those outcomes.
