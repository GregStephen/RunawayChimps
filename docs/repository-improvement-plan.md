...
Start from `codex/fix-level1-crawler-lighting` for this focused Level 1 pass. Run the offline source validator and both Runaway Chimps Editor validators in Unity 2022.3.55f1 before Play Mode. Resolve any compile/import/missing-script issue first. In Level 1, verify the brighter baseline in the headset, Zombie Crawl world scale and floor/hand contact, continuous crawl animation rather than frozen-pose sliding, patrol/chase speeds, both safe-room boundaries in both directions, capture from head/body/hand contact including held overlap, body-position card drops, immediate patrol return, safe-room doorway animation visibility, audio state and the narrowest vent turns. Then repeat the Crawler checks with two Photon clients, including controller handover. The local vent headlamp remains optional/proposed and should not be used to hide a globally unreadable lighting baseline. Continue with the existing Hub/Level 2 travel, reconnect/pause and broader reliability checks after this Level 1 checkpoint is dependable.
