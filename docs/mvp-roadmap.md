...
**#27 investigation, September 21:** Greg approved **Unity 2022.3.62f3 (`96770f904ca7`)** as the isolated compatibility candidate, replacing the earlier 62f2 suggestion. The [Unity runtime security review](unity-runtime-security-review.md) corrects the overbroad Quest vulnerability assumption, records the ordinary 2022.3 support-lifecycle limitation and unresolved exact Meta/XR compatibility, and provides installation, validation and rollback steps. Main and its CI remain pinned to 2022.3.55f1; the isolated branch will intentionally change its own pin when created. Unity/build/headset evidence is still pending in #27.

## Latest sequencing correction and work available now

**Confirmed instruction, September 21:** Greg will test and merge his chosen existing branches first. Do not create the 62f3 compatibility branch until Greg confirms those merges. Then re-read live main and PR state and create the isolated editor trial from the agreed merged SHA. Approval of the version is not permission to ignore this later hold. No gameplay PR is merged automatically and no background branch creation is configured.

**Preparation started in #28:** the [branch testing and build-evidence guide](branch-testing-and-build-evidence.md), [test/build record](templates/test-build-record.md), [defect report](templates/defect-report.md), and [minor-issue signoff register](templates/accepted-minor-issues.md) are prepared on `docs/solo-mvp-roadmap` / PR #49. They can organize current-branch testing without touching gameplay, packages, scenes, editor pins or CI. This is documentation/template preparation only, not an executable build pipeline, a diagnostics feature or an executed test result.

**Next gates:** complete #27 on the agreed merged baseline, finish #28's repeatable local procedure/build identification, gather remaining #29/#30/#31 startup/card evidence, and run #32's exact-combined-build regression. If the corresponding PRs are already merged, do not merge or reimplement them a second time; validate the combined result. #33 then owns travel/capture/interruption acceptance before safe external delivery in #34. Earlier branch passes remain useful historical evidence but do not automatically validate the new editor or combined candidate. Final shop art and Unity 6 are not prerequisites to that first safe tester build.
