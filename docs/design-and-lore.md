...
| Crawler and difficulty | Validate the Zombie Crawl follow-up branch for size, crawl cadence, 12 m prototype detection, contact/held-overlap capture and both safe-room boundaries. Start with one card; add a second only if playtests justify a distinct route and story purpose. |
...
1. Validate `codex/fix-level1-crawler-lighting`: Level 1 stays dark but readable; Zombie Crawl keeps authored world scale, visibly crawls during patrol/chase, pursues eligible vent players, both safe-room routes correctly end/restore eligibility, local-rig contact and held overlap capture only in the vents, dropped cards use the Gorilla body position, and the shared visual remains animated when visible from a safe room.
