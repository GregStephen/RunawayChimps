...
## September 12 Crawler animation-root drift correction
...
**Confirmed source self-review correction before headset retest:** the first in-place implementation still counter-translated the `Zombie Crawl` transform itself. Because that same imported hierarchy can own the Animator, this left two systems writing positional state on the same transform and made the diagnostic measure mostly residual frame-to-frame correction rather than the clip's total internal drift. That version is superseded before Greg's next test. The hardened hierarchy is now `Crawler gameplay root -> CrawlerVisualAnchor -> CrawlerMotionCompensation -> Zombie Crawl`. `CrawlerVisualAnchor` owns whole-visual heading only; the new non-animated `CrawlerMotionCompensation` parent owns pivot/floor alignment and absolute in-place translation only; and the Animator keeps exclusive ownership of the imported Zombie root/skeleton. `CrawlerBodyPathFollower` measures `mixamorig:Hips` relative to the compensation wrapper every rendered frame and assigns the wrapper from its calibrated position minus the **total horizontal X/Z travel drift**. Greg's next screenshot/log pass confirmed horizontal compensation was active (1.22 m attempted horizontal travel), but the core body still rendered below the vent floor. Upward crawl/bob remains Animator-owned; only negative Y root drift is now cancelled at the calibrated floor pose so the whole Zombie cannot sink beneath the vent floor. This uses Hips/root drift rather than full renderer bounds so a low hand cannot lift the entire monster. It never repositions `Zombie Crawl` itself or writes Hips/spine/chest transforms. `CrawlerVisualHeadingStabilizer` now explicitly targets the stable visual anchor rather than inferring `VisualRoot.parent`, so adding the compensation layer cannot make heading and translation fight over one transform. Hand contact IK still runs afterward at order 350.

**Second pre-test source review:** the saved Level 1 gameplay root still carries MiniGamesKid-era orientation data even though the Zombie replacement no longer uses that rig. Source inspection found a legacy 180-degree root roll/tilt plus a serialized 180-degree `modelForwardOffset`; the NavMeshAgent base offset itself is `0`, so the latest low-body report is not explained by agent base offset. The previous setup gave `CrawlerVisualAnchor` local identity, which could still inherit the old gameplay-root tilt during the **initial** Animator/pivot/floor calibration and only become world-upright later when the heading stabilizer ran. PR #15 now clears the old navigation model offset before calibration and explicitly makes `CrawlerVisualAnchor` world-upright from the gameplay travel heading before `CrawlerBodyPathFollower.Configure()` measures Hips and floor contact. The later heading stabilizer maintains that same world-upright ownership.

The same review also resolves the red Console error from Greg's prior log capture: runtime cleanup tried to destroy the obsolete root `Animator` while the old `RigBuilder` still required it. Runtime cleanup now disables marker-confirmed legacy `RigBuilder`/`BoneRenderer` support plus the old root Animator instead of issuing a dependency-invalid destroy. The explicit Editor cleanup removes those dependency components first and only then removes the obsolete root Animator. This is source hardening only; the next Play Mode/headset run must confirm no dependency error, correct initial height, and stable floor contact.

**Pending validation:** Unity 2022.3.55f1/headset must prove the Zombie remains colocated with the selected Crawler parent through several complete crawl cycles while stationary, patrolling, chasing, turning, and after Photon/controller handoff. The body may still have the separately documented rigid long-body corner-fit limitation, but it must no longer walk/crawl meters away from the gameplay root or snap back on animation-loop boundaries. The horizontal-travel warning plus a new downward-floor-sink warning are diagnostic only; repeated visible separation or body-floor penetration remains a failed test. Source validation is necessary evidence only and does not mark this runtime defect resolved.

## September 13 Crawler size and full-body vent containment follow-up

**Confirmed runtime result:** the post-cleanup/in-place Crawler is materially better in Greg's latest Play Mode pass. The Console capture no longer shows the prior `Can't remove Animator because RigBuilder...` error, and the Crawler initialization/cleanup/contact systems complete. A remaining visual-fit failure is that the Zombie is still too large for the Level 1 ducts: Greg tested the visual at **0.6 scale instead of 1.0**, found that size materially better, and observed the head still pulling outside the vent shell. Treat 0.6 as the current confirmed Level 1 Crawler visual-scale target; full vent containment remains runtime-pending.

**Implemented on PR #15:** `CrawlerVisualController` now applies the confirmed 0.6 visual multiplier in code so a stale/default 1.0 value cannot silently restore the oversized presentation. `CrawlerSurfaceContactIK` now resolves both Mixamo arm chains **and both leg chains**; hands and feet use collision-aware two-bone solving against static vent geometry, with the same teleport/controller-handoff state reset and solved-position caching. New `CrawlerVentContainment` runs after heading + animation-root/floor compensation and before limb IK. It uses bounded sphere probes on the head, chest, hips, elbows and knees, sweeps from the last rendered safe positions to catch thin-wall crossings, resolves current overlap with `Physics.ComputePenetration`, and requests one common rigid world-space correction through `CrawlerBodyPathFollower.SetVentContainmentOffsetWorld(...)`. No head/chest/hips/spine/elbow/knee bone is directly translated or rotated by containment.

**Containment boundary:** the intent is that the representative head/core/joint probes plus hand/foot IK keep the visible monster inside the duct in normal Level 1 play. This is deliberately **not** a skinned-mesh physics/ragdoll conversion and does not mathematically test every rendered vertex; that would be too expensive and would reintroduce Animator/physics ownership conflicts. The rigid containment correction is capped at 0.32 m and logs if that limit is reached, because reaching the cap means the current animation/route/scale still cannot fit cleanly and should be treated as a failed test rather than forcing the skeleton. Long-torso 90-degree corner shaping is still a separate rig-safe problem.

**Pending validation:** Unity 2022.3.55f1/headset must verify the 0.6 Zombie stays readable/imposing while the head/core/elbows/knees remain inside vent walls/floor/ceiling, both feet stop/bend against encountered surfaces without knee inversion or jitter, existing hand contact still works, the rigid containment offset releases smoothly when clear, and capture/Photon/navigation behavior remains aligned with the authoritative gameplay root.

## September 13 Level 1 keycard-room and regeneration-lore direction

**Confirmed creative direction:** the currently sparse Level 1 keycard safe room should become a **regeneration/tissue-damage testing laboratory**, not another surveillance/monitor room and not a conventional operating theater. Its purpose is to reveal what the facility does to animal subjects generally and to make finding the fixed personal staff-access card feel like environmental discovery rather than floor loot.

**Confirmed story boundary:** this room is **not** a second Crawler escape origin. The established Level 1 story remains the damaged Crawler cage, scratches/blood leading from that cage, and the Crawler entering the vent system from the containment area. Do not add a broken restraint, blood trail, or other staging that implies the Crawler escaped from the keycard room as well.
...
| --- | --- | --- |
| 2026-09-13 | Replace the empty/monitor-room-style Level 1 keycard-room concept with a regeneration/tissue-damage testing laboratory. It is a safe objective room that reveals the facility's broader experiment program, not another Crawler escape site. The Crawler's exposed bone/spinal damage and nearly severed legs fit repeated tissue/nerve/bone damage plus attempted regeneration rather than one simple leg-removal operation. | Confirmed creative direction. Environment layout/art and exact staff-card presentation are not implemented; exact props, labels, trial identity and any specific subject-to-room link remain open. |
...
| 2026-09-12 | The Crawler's path-following presentation must preserve the Zombie Crawl skeleton's authored body proportions. Do not reposition or independently rotate hierarchical chest/spine/hips bones after Animator evaluation; both attempted torso-deformation approaches failed in headset. Keep the core torso Animator-owned; bounded two-bone hand/foot IK plus one rigid body-containment offset are the approved environmental-contact exceptions. | Confirmed correction from Greg's squished/folded-body runtime reports and September 13 vent-fit retest. PR #15 now uses rigid whole-visual alignment/heading, 0.6 visual scale, hand/foot surface-contact IK and head/core/joint containment probes; long-body corner bending remains pending a separate rig-safe solution. Unity/headset validation pending. |
...
| Confirmed | The Level 1 keycard safe room is a regeneration/tissue-damage testing laboratory rather than another surveillance/monitor room. It reveals the facility's broader experiment program without creating a second Crawler escape origin; the Crawler's established escape remains cage → trail → vent. |
| Confirmed | The Crawler's exposed bone, deep tissue loss, exposed spinal area and nearly severed legs fit repeated tissue/nerve/bone damage plus attempted regeneration rather than a single simple leg-removal operation. Exact trial identity and whether the keycard room treated this specific Crawler remain undefined. |
...
The player characters are laboratory gorillas trying to escape the facility that experiments on animals. The overall objective is to get outside the lab. The regeneration/tissue-damage program revealed in Level 1 is the first confirmed experiment family; the broader facility history and the purpose/origin of later experiments remain to be written. Each level has its own monster, objective, and environmental clues.
...
### Regeneration and tissue-damage testing

**Confirmed September 13, 2026:** Level 1 establishes that the facility performs destructive tissue/nerve/bone experiments intended to test or force regeneration. This is broader than a conventional surgical program: subjects may have tissue removed or damaged, exposed bone or nerves, and incomplete or malformed regrowth. The Crawler's visible wounds are consistent with that program, but the story does not currently assert a specific trial number, procedure sequence, or that the Level 1 keycard room was the exact room where this individual Crawler was treated.

Use environmental clues rather than a large exposition dump. Diagrams, treatment equipment, sample storage, damaged-subject anatomy references and terse trial labels can communicate the program. Exact terminology, compounds, staff names, subject numbers and written documents remain open until the room dressing/lore pass.
...
Desired theme: include dark humor about the experiments and the players having no legs. Proposed explanation: researchers tried to restrict mobility through limb removal, but the subjects adapted to powerful arm-based movement. This explanation is not yet canon and is separate from the Crawler's now-confirmed regeneration/tissue-damage direction.
...
| Level1_Containment | Cage room, vent maze, Crawler, regeneration/tissue-testing keycard safe room, keycard objective, and exit to Level 2. |
...
Random keycard locations and audio from the security monitors have also been superseded. The card has a fixed starting spot, and all monitors are silent. A September 13 room-design correction also supersedes using another monitor/security room as Level 1's keycard destination; that destination is now the regeneration/tissue-damage testing safe room.
...
Follow the marks from the safe cage room into a vent maze with dead ends and two paths into one **regeneration/tissue-damage testing keycard safe room**. Each player has their own card at the same fixed starting spot. The Crawler stays in the vents; both rooms are safe. Either vent entrance can be used for the return trip. Use your card at the cage-room exit: hear the door, fade to black, and arrive safely in Level 2. Billy stays in Level 1 and sees you disappear; your completion does not open a shared passage for him.

The keycard destination should read as a real laboratory workspace rather than an empty pickup room: it reveals the facility's regeneration/tissue-damage program while preserving the current objective rules. It must not imply the Crawler escaped from this room; the damaged cage and cage-to-vent trail remain the Crawler's escape story. Exact prop layout and card presentation remain to be authored.
...
**Confirmed September 13 correction:** do not extend that escape trail into or out of the keycard room. The regeneration/tissue-damage laboratory is broader facility storytelling, not the Crawler's second origin/escape point. Its equipment may help players understand the kind of destructive experimentation that could have produced the Crawler's exposed bone, spinal damage and near-severed legs, but the room should not claim a specific treatment history for this individual unless that is decided later.
...
- Keycard room: show the card at its fixed starting spot beside a recognizable regeneration/tissue-testing feature. The image teaches where it belongs; it does not track a card dropped elsewhere during an attempt.
...
| Level 1 keycard-room dressing | Regeneration/tissue-damage testing is confirmed. Choose the exact prop set, staff-card presentation, trial-label terminology, lighting and whether any clue explicitly links a subject number to the Crawler; do not imply a second escape route from this room. |
...
## Latest correction — Crawler surface-contact IK (September 12, 2026)

**Confirmed / implemented on PR #15:** Greg confirmed that the Crawler's animated hands should treat the vent shell as a physical boundary instead of visually passing through walls, floor, or ceiling. `CrawlerSurfaceContactIK` now runs after the authored crawl animation and whole-visual heading. Each hand is treated as a small collision sphere; when the animated wrist would cross static same-scene geometry, the target is clamped to the first surface and a two-bone solve rotates only that arm's upper-arm and forearm so the hand remains at the vent surface. Dynamic Rigidbody objects, players, keycards, triggers, and the Crawler's own hierarchy are excluded from this contact pass.

This **supersedes the earlier blanket wording that no Zombie bone may ever be written after Animator evaluation**. The active rule is narrower: core torso/spine/hips remain Animator-owned and must not be directly repositioned or independently rotated by path-following code, because both earlier torso approaches failed in headset. Arm-only post-Animator IK is explicitly allowed for bounded environmental contact, provided it preserves bone lengths and does not move the gameplay/NavMesh/Photon root. `CrawlerBodyPathFollower` remains rigid and does not manipulate bones.

**Implementation details / pending validation:** the prototype uses a 5.5 cm hand contact sphere with a small surface clearance, hard-clamps on first contact, preserves the Animator's current elbow bend side as the IK pole, restores the authored hand orientation after solving, and blends back to pure animation when the hand clears the surface. Source validation covers the dedicated IK path and forbids torso/root manipulation there. Unity 2022.3.55f1/headset still must prove that both hands stop cleanly on vent walls/floor/ceiling without elbow flips, jitter, excessive stretching, or false contact while crawling straight and through turns. This limb-contact system does **not** by itself solve the separate long-torso 90-degree-corner problem.
...
## September 13 launch-terminal CRT/static source validation
...
## September 13 physical launch workstation refinement
...
## September 14 launch vignette and multiplayer Hub-spawn correction

**Confirmed correction:** the launch workstation is a private loading vignette, not a physical Hub workstation/arrival location. Each client sees only its own temporary desk/monitor presentation. Startup must end with an opaque black cover, removal/unload of that presentation, and then a reveal of the player at their actual Hub spawn. The workstation never persists into shared Hub gameplay.

The startup set may follow hidden `XROrigin` root relocation so the desk stays perceptually stable while the persistent rig is snapped behind black/loading presentation; it must not follow ordinary head look or become head-locked. This supersedes the earlier absolute-world-stationary interpretation that could leave the desk behind when the XR origin moves.

**Confirmed multiplayer requirement:** simultaneous players should not all materialize at one `HubSpawn`. The Hub should provide distinct cold-start spawn slots for the current 10-player session cap. Slot assignment happens after Photon room join and before reveal; startup workstations are local-only and do not participate in network spawning. Existing route-specific Hub return markers remain separate from the cold-start slot pool. Exact allocation code/marker layout is pending implementation.
...
## September 21 launch presentation correction — restore original screen-space boot

**Failed runtime validation / latest explicit correction:** Greg retested the 2.50 m world-space green terminal and reported that the terminal and perceived player view visibly dropped during startup as the XR rig settled, the hands could be heard striking the ground, and the static treatment was not perceptible enough. Greg explicitly asked to bring the green screen **back to where it was before**.

**Confirmed superseding presentation:** restore the approved PR #20-style flat green security boot as **Screen Space - Camera** UI on the Loading canvas. There is no world-space terminal distance, XR-origin-parented panel, physical workstation, or launch-panel transform. The terminal uses the original view-relative sizing (approximately 72% of view width / 84% height) and the original camera-plane placement. Hidden rig grounding/snapping may continue behind the opaque Loading presentation, but it must not move the boot on screen.

**Implemented on `feature/launch-presentation-polish` / PR #21, pending runtime retest:** `SecurityBootPresentation` again builds the terminal directly under the Loading canvas and binds that canvas to the tracked XR camera with `RenderMode.ScreenSpaceCamera`. The superseded `SecurityWorkstationVignette` helper and metadata were removed. CRT noise/scanlines/interference remain; a restrained startup/interference static crackle was added so short startups can still communicate the display effect. `HandImpactAudio` now suppresses synthetic impacts while the cold-start Loading presentation is active, preventing hidden spawn/grounding motion from sounding like player-authored hand slaps.
...
**Implemented tuning candidate on `feature/launch-presentation-polish` / PR #21:** retain the 1080 × 820 terminal layout and all readiness/CRT/retry behavior, but set the approved `terminalScale = 0.50`, producing approximately **36% width / 42% height** from the original **72% width / 84% height** PR #20 baseline and remains centered. No world-space distance or XR-origin panel is reintroduced.
