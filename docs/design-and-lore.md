...
**September 12 Hub surveillance implementation:** Greg confirmed that the Hub security monitors should be reusable physical displays whose sector name lives inside the screen content rather than on the bezel. The surveillance presentation uses clean captured 4:3 level stills with grain/scanlines and dynamic sector text applied on the Hub display; it does not keep remote level scenes loaded merely to render live camera feeds. Draft PR #16 on `feature/hub-surveillance-monitors` now source-implements data-driven sector feeds, per-monitor footage/grain/text presentation, full-sequence rotation when there are more level feeds than physical monitors, preservation of each feed's loop/scare state while off-screen, and a Unity Editor 4:3 still-capture tool. Source integrity passes for PR #16. The external monitor model, authored monitor prefab/world-space Canvas, captured stills, feed assets and `Hub_Base` placement are not yet committed, and Unity 2022.3.55f1/headset/Quest validation remains pending. See [Hub security room](hub-security-room.md) and [Hub surveillance monitor setup](surveillance-monitor-setup.md).
