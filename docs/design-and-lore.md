...
Pending validation: first prove `mixamo.com.anim` visibly deforms Zombie Crawl and that the persistent binding diagnostic does not fire. Verify chest/root alignment and render-bounds floor contact in the narrowest straight; confirm the seeded body starts extended, bends through a 90-degree corner and junction without leaving the vent, and does not produce unacceptable arm stretching from hand containment. Then confirm patrol/chase cadence, both safe-room entrances/exits, ordinary head/body/hand contact and held-overlap capture only in the vents, body-position card drops, respawn/controller handover, safe-room doorway animation visibility, and repeat on two Photon clients and target Quest hardware.
