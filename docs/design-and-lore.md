...
**September 12 spawn-hand runtime correction:** Greg reported that a player can spawn with one or both hands stuck in the floor unless the physical hands/controllers are held up during spawn. This is a separate failure from the body-capsule floor-penetration case and is not an acceptable player requirement. Source inspection found that `GorillaLocomotion.Player.ResetAfterTeleport()` seeded the virtual hand followers and cached hand positions directly from the tracked controller poses, so a low or below-floor pose could become the first locomotion contact. Branch `fix/spawn-hand-floor-stick` now sweeps each reset hand from the arm root toward its tracked pose using the existing locomotion collision path. A meaningfully obstructed hand starts at the first safe contact and is temporarily excluded from locomotion pushing until the tracked pose returns to reachable free space; ordinary low or surface-touching poses are preserved by a small tolerance. The shared reset path covers Hub startup, sector travel, capture respawn and floor-recovery resets. PR #14's Source validation passes for the source implementation, but Unity 2022.3.55f1/headset validation remains pending and the defect is not considered resolved until hands-down spawn/re-entry tests pass.
...
| Confirmed | Spawn, sector travel and capture respawn must not require the player to hold their physical hands/controllers up. Low tracked hand poses must not seed locomotion from inside floor/wall geometry or create a first-frame launch/pin; an invalid hand should remain at a safe reachable boundary until tracking returns it to valid space. |
...
| Confirmed | The Crawler's visible long body must remain aligned to the vent route and bend around corners rather than behaving like one rigid rotating child. The crawl clip supplies limb motion; visual path correction may bend torso bones locally without changing gameplay/Photon authority. |
...
- Validate spawn/travel/respawn with controllers in natural low poses. No route may require hands to be raised before placement; an initially obstructed hand must remain safely clamped without pinning/launching the body and resume normal locomotion contact once the tracked pose becomes reachable.
...
**Latest Crawler visual correction:** Greg's runtime test showed the previous animation-speed/restart work still produced a visually static, offset, rigid Crawler. That result supersedes any earlier implication that crawl presentation was fixed. Commits `5f5ea84` and `97b20e6` add a dedicated `CrawlerVisualAnchor` plus `CrawlerBodyPathFollower`. The visual is calibrated from a chest/spine/hips bone rather than the imported object pivot, grounded from its render bounds, and its torso samples a seeded history of the gameplay root's vent-centerline motion so the front can enter a corner while the hips remain in the prior corridor. Animated hands are bounded against encountered vent surfaces. The crawl controller now explicitly enters `mixamo_com`, keeps the Animator uncullable, probes limb-bone motion while the root moves, attempts one rebind/restart when state time advances without bone motion, and logs a persistent clip/skeleton binding error if that still fails. Navigation, capture, audio and Photon continue to own only the gameplay root. These are implemented source corrections, not yet validated Unity/headset behavior.

Pending validation: first prove `mixamo.com.anim` visibly deforms Zombie Crawl and that the persistent binding diagnostic does not fire. Verify chest/root alignment and render-bounds floor contact in the narrowest straight; confirm the seeded body starts extended, bends through a 90-degree corner and junction without leaving the vent, and does not produce unacceptable arm stretching from hand containment. Then confirm patrol/chase cadence, both safe-room entrances/exits, ordinary head/body/hand contact and held-overlap capture only in the vents, body-position card drops, respawn/controller handover, safe-room doorway animation visibility, local headlamp readability/Quest cost, and repeat on two Photon clients and target Quest hardware.
...
| Crawler and difficulty | Validate the latest Zombie Crawl body-path implementation for visual/gameplay-root alignment, real limb animation binding, seeded body extension, 90-degree/junction bending, hand containment, 12 m prototype detection, contact/held-overlap capture and both safe-room boundaries. Start with one card; add a second only if playtests justify a distinct route and story purpose. |
...
1. Validate PR #14's spawn-safe hand reset together with the strengthened XR/body grounding. Cold-start into the Hub repeatedly with both hands naturally down, one controller low, both controllers low/on the lap, standing, crouching and after recentering. The virtual hands must never become permanently buried or pin/launch the player; a hand clamped by geometry must move along the safe boundary and resume normal locomotion contact once lifted into reachable space. Repeat Hub → Level 1, Level 1 → Hub, Level 1 completion → Level 2, RETURN TO SECURITY and Crawler capture respawns. Watch for `[RigFloorPenetrationGuard]` recovery warnings: one warning identifies a late XR/physics displacement that the guard recovered, while repeated warnings remain a failed validation and require deeper tracking-origin/rig-ownership work.
...
1. Validate `codex/fix-level1-crawler-lighting`: Level 1 stays dark but readable; Zombie Crawl keeps authored world scale; the chest/visual anchor stays on the gameplay path; `mixamo_com` visibly moves limb bones while the root travels; the body begins extended and follows the recorded path through a 90-degree turn/junction without leaving the vent; hands do not create unacceptable wall penetration/stretching; pursuit/safe-room/contact capture behavior still works; dropped cards use the Gorilla body position; and the shared visual remains animated when visible from a safe room.
