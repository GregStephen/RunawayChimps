...
public static class SectorTravelValidator
{
    ...
    public static void Validate()
    {
        ...
        for (int i = 0; i < ScenePaths.Length; i++)
        {
            ...
            if (enabled.Length <= i || enabled[i] != path)
                errors.Add(path + " must be enabled at index " + i);
        ...
        if (errors.Count == 0)
            ...
            foreach (var error in errors)
                Debug.LogError("[Sector travel] " + error);
    }

    private static void ValidateScene(string path, List<string> errors)
    {
        // Unity 2022.3.55f1 does not expose EditorSceneManager.OpenPreviewScene.
        // Open only scenes that are not already loaded, validate them additively,
        // then close only what this validator opened so the user's scene setup and
        // unsaved edits in already-open scenes are left alone.
    ...
    private static void CheckScene(Scene scene, List<string> errors)
    {
        ...
        if (scene.name == "Bootstrap")
        {
            ...
            ValidateLocalPressColliders(scene, errors);
            return;
        }
        ...
        {
            errors.Add(scene.name + " requires one SectorScene.");
            return;
        }
    ...
    private static void ValidateLocalPressColliders(Scene scene, List<string> errors)
    {
        int fingerTipLayer = LayerMask.NameToLayer("FingerTip");
        if (fingerTipLayer < 0)
        {
            errors.Add("Project requires a FingerTip layer for physical hand buttons.");
            return;
        }

        var fingerTips = Find<Collider>(scene).Where(c =>
            c.gameObject.layer == fingerTipLayer &&
            c.CompareTag("HandTag") &&
            c.GetComponentInParent<LocalRigMarker>() != null).ToArray();

        if (fingerTips.Length < 2)
            errors.Add("Bootstrap requires tagged local FingerTip colliders for both hands so physical buttons work in headset and editor testing.");
    }
    ...
    private static void ValidateHubEntranceButton(Scene scene, SectorDoor hubDoor, List<string> errors)
    {
        ...
        if (hubDoor != null && !string.IsNullOrWhiteSpace(hubDoor.label))
            Debug.LogWarning("[Sector travel] Hub Level 1 SectorDoor still has an obsolete direct-select label serialized. It is ignored at runtime; clear the label when editing Hub_Base next.");
        ...
        int fingerTipLayer = LayerMask.NameToLayer("FingerTip");
        int fingerTipMask = fingerTipLayer >= 0 ? 1 << fingerTipLayer : 0;
        ...
        if (requireLocalRig == null || !requireLocalRig.boolValue ||
            ...
            pressLayers == null || fingerTipMask == 0 || (pressLayers.intValue & fingerTipMask) == 0)
            errors.Add("StartLevel1Button must filter presses to the local tagged FingerTip layer.");
